...
void DynamicalStateData::create_attr(const std::string& name, const int def)
{
...
void DynamicalStateData::create_attr(const std::string& name, const float def)
{
...
int DynamicalStateData::id(const size_t p) const
{
   return ids_->second.get(p);
}

float DynamicalStateData::mass(const size_t p) const
{
   return masses_->second.get(p);
}

float DynamicalStateData::rad(const size_t p) const
{
...
void DynamicalStateData::set_attr(const std::string& name, const size_t p, const int value)
{
   int_attributes_[name].set(p, value);
}

void DynamicalStateData::set_attr(const std::string& name, const size_t p, const float value)
{
   float_attributes_[name].set(p, value);
}

void DynamicalStateData::set_attr(const std::string& name, const size_t p, const Vector& value) 
{
   vector_attributes_[name].set(p, value);
}

void DynamicalStateData::set_attr(const std::string& name, const size_t p, const Color& value) 
{
...
pba::AABB BoundingBox(const DynamicalStateData& d)
{
   ...
   {
      const Vector& position = d.pos(i);
      if (position[0] < llc[0] ) { llc[0] = position[0]; }
      if (position[1] < llc[1] ) { llc[1] = position[1]; }
      if (position[2] < llc[2] ) { llc[2] = position[2]; }
      if (position[0] > urc[0] ) { urc[0] = position[0]; }
      if (position[1] > urc[1] ) { urc[1] = position[1]; }
      if (position[2] > urc[2] ) { urc[2] = position[2]; }
   }
