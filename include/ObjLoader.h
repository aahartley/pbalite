...
{
  Triangle(){}
  Triangle(size_t vv1, size_t vv2, size_t vv3) : v1(vv1), v2(vv2), v3(vv3)
  {
    // e1 = v2 - v1;
    // e2 = v3 - v1;
    // e3 = v3 - v2;
...
class ObjLoader {
public:
    //store indices into tris
    static void loadObj(std::vector<Vector>& vertices, std::vector<Triangle>& triangles, const std::string& filename) {
        ...
        while (std::getline(file, line)) {
            ...
            if (token == "v") {
                ...
                triangles.push_back(Triangle(v1Index-1,v2Index-1,v3Index-1));
                //std::cout << v1Index << ' ' << v2Index << ' ' << v3Index << ' ' << vertices[v1Index].X() << ' ' << vertices[v1Index].Y() << ' ' << vertices[v1Index].Z() << '\n';
            }
        }

        file.close();
    }
    //store verts in tri directly
    static std::vector<TriangleV> loadObjTri(const std::string& filename) {
        ...
        while (std::getline(file, line)) {
            ...
            if (token == "v") {
                ...
                triangles.push_back(TriangleV(v1,v2,v3));
